...
export default function Kutuphane() {
  ...
  // Yeni Kitap Seçimi (Dosyayı bellekte tutar, veriye çevirmez)
  ...
  return (
    <div className="lib-wrapper">
          ...
          <div className="form-card">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="file-upload-section">
                ...
                {file && <span className="file-badge">{(file.type.includes('pdf') ? 'PDF' : 'WORD')} Seçildi</span>}
              </div>

              <button type="submit" disabled={uploading} className="save-btn">
                {uploading ? '☁️ Yükleniyor... Lütfen bekleyin.' : 'Buluta Yükle'}
              </button>
            </form>
          </div>
        )}
        ...
        <div className="modal-backdrop" onClick={() => setSelectedBook(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="pdf-viewer">
              {/* iframe artık doğrudan Supabase linkini gösteriyor */}
              <iframe 
                src={`${selectedBook.file_url}#page=${currentPage}&toolbar=0&view=FitH`} 
      ...
      <style jsx>{`
        ...
        .save-btn:disabled { background: #4c1d95; cursor: not-allowed; }
        ...
        @media (max-width: 768px) { .modal-header { flex-direction: column; align-items: flex-start; } .page-controls { width: 100%; justify-content: space-between; } }
        @media (max-width: 600px) { .input-row { grid-template-columns: 1fr; } .hide-mobile { display: none; } .books-grid { grid-template-columns: 1fr 1fr; gap: 15px; } .book-cover { height: 180px; } }
      `}</style>
    </div>
  );
}